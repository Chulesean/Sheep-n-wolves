...
def run_game(screen, screen_w, screen_h, cx, cy, vs_bot=False):
    ...
    :param vs_bot: Whether the game is played against a bot
    :return: True to return to menu, False to exit application
    """
    ...
    while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if result == 'exit':
                    ...
                    renderer = GameRenderer(
                        assets, board, game_state, screen, screen_w, screen_h
                    )
                    controller = GameController(assets, board, game_state)
        ...
        # Bot turn (black always plays as the bot)
...
def main():
    ...
    Handles menu navigation and game lifecycle.
    """
    pygame.init()
        ...
        if selected is not None:
            if selected == 0:
                ...
                continue_game = run_game(
                    screen, screen_w, screen_h, cx, cy, False
                )
                ...
                # Player vs Bot
                continue_game = run_game(
                    screen, screen_w, screen_h, cx, cy, True
                )
                if not continue_game:
                    running = False
